...
CREATE TABLE [dbo].[TestCase](
	...
	[LastRefTestRunDate] [datetime] NOT NULL
) ON [PRIMARY]
GO
...
MERGE [dbo].[ChangeRequest] AS TARGET      
...
MERGE [dbo].[ChangeRequest_KPI] AS TARGET      
...
MERGE [dbo].[DeliveryTracking_KPI] AS TARGET      
...
MERGE [dbo].[QualityOfEstimates] AS TARGET      
USING CTE1  AS SOURCE       
...
MERGE [dbo].[QualityOfEstimates_KPI] AS TARGET      
USING CTE2  AS SOURCE       
ON (TARGET.[ProjectId] = SOURCE.[ProjectId] )        
WHEN MATCHED AND (TARGET.ProjectId = SOURCE.ProjectID)     
OR TARGET.ProjectID <> SOURCE.ProjectID  
...
SELECT 
  ...
  INNER JOIN [10.1.22.94].[Tfs_Warehouse].[dbo].[vFactLinkedCurrentWorkItem] AS FLCWI
...
FROM [10.1.22.94].'+@DBCollection+'.[dbo].[tbl_Suite] With(Nolock)


BEGIN
Insert INTO [ETL].[TestSuiteLink]  
SELECT  ' + @ProjectID + ' as ProjectID,* 
FROM [10.1.22.94].' + @DBCollection + '.[dbo].[tbl_SuiteEntry]   With(Nolock)
END
...
--***********************************    
MERGE [dbo].[Bug] AS TARGET    
USING [Etl].[Bug]  AS SOURCE     
...
--refreshed data from source table        
--***********************************        
MERGE [dbo].[Epic] AS TARGET        
USING [ETL].[Epic]  AS SOURCE         
...
--refreshed data from source table      
--***********************************      
MERGE [dbo].[Feature] AS TARGET      
USING [ETL].[Feature]  AS SOURCE       
...
--refreshed data from source table        
--***********************************        
MERGE [dbo].[Iteration] AS TARGET        
USING [ETL].[Iteration]  AS SOURCE         
...
--refreshed data from source table    
--***********************************    
MERGE [dbo].[UserStory] AS TARGET    
USING [ETL].[UserStory]  AS SOURCE     
